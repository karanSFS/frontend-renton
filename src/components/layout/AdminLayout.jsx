...
import { LayoutDashboard, Users, Car, LogOut, Settings, BarChart2, Calendar } from 'lucide-react';
...
const AdminLayout = () => {
    ...
    useEffect(() => {
        console.log('AdminLayout Effect Check:', { isAuthenticated, user, role: user?.role });
        ...
        if (!user) {
             console.warn('AdminLayout: No User Object -> Redirecting to Login');
             navigate('/admin/login', { replace: true });
             return;
        }

        if (user.role !== 'admin') {
             console.warn(`AdminLayout: User role is '${user.role}', not 'admin' -> Redirecting to Login`);
             navigate('/admin/login', { replace: true });
        }
    ...
    if (!isAuthenticated || !user) {
        return <div className="h-screen bg-black flex items-center justify-center text-white">Checking credentials...</div>;
    }

    // If authenticated but not admin, show Access Denied instead of redirecting loop
    if (user.role !== 'admin') {
        console.warn(`AdminLayout: User role is '${user.role}', not 'admin'`);
        return (
            <div className="h-screen bg-black flex flex-col items-center justify-center text-white space-y-4">
                <h1 className="text-3xl font-bold text-red-500">Access Denied</h1>
                <p className="text-gray-400">Your account ({user.email}) is not an administrator.</p>
                <div className="p-4 bg-dark-900 rounded border border-white/10 text-xs font-mono text-gray-500">
                    Current Role: {user.role || 'undefined'}
                </div>
                <button 
                    onClick={handleLogout}
                    className="px-6 py-2 bg-gold-500 text-black font-bold rounded hover:bg-white transition-colors"
                >
                    Logout
                </button>
            </div>
    ...
    const navItems = [
        ...
        { path: '/admin/bookings', label: 'Bookings', icon: Calendar }, // Added Bookings
